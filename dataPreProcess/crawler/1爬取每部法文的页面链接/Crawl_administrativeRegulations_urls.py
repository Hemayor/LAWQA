...
with sync_playwright() as p:
    ...
    # 3. 点击“行政法规➕” → “行政法规”
    page.wait_for_selector("#EffectivenessDicport_10_switch", timeout=60000)
    page.locator("#EffectivenessDicport_10_switch").click()
    time.sleep(1)

    page.wait_for_selector("#EffectivenessDicport_11_span", timeout=60000)
    page.locator("#EffectivenessDicport_11_span").click()
    time.sleep(1)
...
# 9. 保存到文件，带序号
with open("pkulaw_administrativeRegulations_urls.txt", "w", encoding="utf-8") as f:
    for idx, (title, url) in enumerate(all_urls, 1):
        f.write(f"{idx}. {title}\t{url}\n")

print("完成，已保存 pkulaw_administrativeRegulations_urls.txt")
