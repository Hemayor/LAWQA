...
cross_encoder_model.model.eval()

print("✅ 本地模型加载完毕！\n" + "=" * 40)


# ==========================================
# 3. LangChain 工具定义 (双路召回 + 统一精排)
# ==========================================
@tool
def retrieve_and_rerank_tool(state: AgentState) -> Dict[str, Any]:
    """
    作用：本地法律知识库检索与精排。
    逻辑：使用【原问题】和【改写问题】进行双路召回（去重），然后将结果与【原问题】拼接进行精排。
    字段精简：仅返回 id, title, content, rerank_score
    """
    start_time = time.time()

    original_query = state.get('original_request', '')
    rewrite_query = state.get('rewrite_request', '')
    ...
    # A: 构建双路查询请求
    # --------------------------------------------------
    queries_to_embed = [original_query]
    # 如果改写词存在且和原问题不一样，才加入第二路召回，否则没必要浪费算力
    if rewrite_query and rewrite_query != original_query:
        queries_to_embed.append(rewrite_query)
        print(f"  -> 🔍 [召回阶段] 启用双路召回 (原问题 + 改写问题)")
    else:
        print(f"  -> 🔍 [召回阶段] 启用单路召回 (仅原问题)")

    # 批量向量化 (比分两次 encode 更快)
    query_vectors = embedding_model.encode(queries_to_embed).tolist()

    # 批量从 ChromaDB 查询 (返回的是一个二维列表结构)
    ...
    # B: 结果合并与去重
    # --------------------------------------------------
    candidate_docs_dict = {}  # 使用字典的键唯一性进行去重

    if search_results and search_results.get('documents'):
        # 遍历每一路召回的结果
        for i in range(len(queries_to_embed)):
            if len(search_results['documents']) > i:
                docs = search_results['documents'][i]
                metas = search_results['metadatas'][i] if search_results.get('metadatas') else [{}] * len(docs)
                ids = search_results['ids'][i]

                # 塞入字典去重
                for doc, meta, doc_id in zip(docs, metas, ids):
                    if doc_id not in candidate_docs_dict:
                        candidate_docs_dict[doc_id] = {
                            "id": doc_id,
                            "content": doc,
                            **meta
                        }
    ...
    print(f"  -> 📦 [合并阶段] 召回总数去重后获得 {len(candidate_docs)} 个候选法条")

    if not candidate_docs:
        return {"retrieval_results": [], "timing_stats": state.get('timing_stats', {})}

    # --------------------------------------------------
    # C: 精排 (Reranking) - 核心修改：强制使用原问题
    # --------------------------------------------------
    # 【需求对齐】将候选法条与【原问题】拼接进行精排，前512字符截断
    rerank_pairs = [[original_query, doc['content'][:512]] for doc in candidate_docs]
